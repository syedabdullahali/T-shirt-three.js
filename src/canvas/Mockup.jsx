...
const Mockup = () => {
  ...
  return (
    ...
    <div className='flex w-full h-fit'>
      <div className='relative overflow-hidden bg-white h-96 w-96'>
      ...
      <label className='text-lg'><strong>Mockup Size</strong></label>
      ...
      value={mockupAtr.size} style={{width:"100%"}} onChange={(e)=>setMocupAtr(prev=>({...prev,size:e.target.value}))} />
        <label className='text-lg'><strong>Mockup X Position </strong></label>
      <input type={'range'} min='10' max='100' className="mb-8 slider " id='size-slider'  
      value={mockupAtr.mockupXposition} style={{width:"100%"}} onChange={(e)=>setMocupAtr(prev=>({...prev,mockupXposition:e.target.value}))} />
        <label className='text-lg'><strong>Mockup Y Position </strong></label>
      <input type={'range'} min='10' max='100' className="mb-8 slider " id='size-slider'  
      value={mockupAtr.mockupYposition} style={{width:"100%"}} onChange={(e)=>setMocupAtr(prev=>({...prev,mockupYposition:e.target.value}))} />
      <button className='w-full px-6 py-2 mt-8 text-xl text-white bg-blue-400 rounded' onClick={()=>handaleApplyMockeUp()}> Apply Mockup </button>
      <button className='w-full px-6 py-2 mt-4 text-xl text-red-400 border border-red-400 rounded'>Remove Mockup </button>
      </div>
    ...
    <button className='block px-6 py-2 mx-2 mt-4 ml-auto text-xl text-white bg-blue-400 rounded w-fit'>Create Mockup</button>
    <h2 className='font-[600] text-xl my-2'>Thread Texture </h2>
